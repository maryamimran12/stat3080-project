...
#  unknown populations are the same. A permutation test can be used   #
...
sim_diffs_null <- replicate(B, rand.test(length(samp_data)))

## Plot the null hypothesis sampling distribution
diffs_df <- data.frame(sim_diffs_null)
ggplot(diffs_df, aes(x=sim_diffs_null)) + geom_histogram(binwidth=1,fill="white",color="black")

## Determine the p-value (two-tailed)
sum(sim_diffs_null <= samp_diff | sim_diffs_null >= 2*mean(sim_diffs_null) - samp_diff)/B
